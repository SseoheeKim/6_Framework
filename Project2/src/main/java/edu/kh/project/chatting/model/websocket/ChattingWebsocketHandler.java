...
public class ChattingWebsocketHandler extends TextWebSocketHandler{
	...
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {
		// WebSocketSession 
		// - 클라이언트와 서버 간 전이중 통신을 담당하는 객체로
		// 	 웹소켓에 접속한 회원의 HttpSession을 훔쳐서 가지고 있음
		
		sessions.add(session);
		// sessions 내에 현재 채팅방에 접속한 회원의 세션을 모아둠
	}
	...
	// --> handelerTextMessage 수행
	// --->  message.getPayload() == JSON데이터
	@Override
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		
		// 전달 받은 내용은 JSON 형태
		// 전달받은 내용 : {"senderNo":"13","targetNo":"15","chattingNo":"1","messageContent":"ㅇㅇㅇ\n"}
		logger.info("전달받은 내용 : " + message.getPayload());
		
		// Jackson-bind에서 제공하는 ObjectMapper객체 사용
		...
		if(result > 0) {  // 삽입 성공 시 
			...
			// 보낸 시간이 DB에 있지만 msg객체에는 없는 상태(null)
			// -> 보낸 시간 생성
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy.MM.dd hh:mm");
			msg.setSendTime(sdf.format(new Date()));
			
			// msg객체(채팅방 번호, 대상 내용, 내용, 보낸 사람 번호, 보낸 시간
			// -> JSON으로 변환하여
			// -> 로그인 한 회원 중 대상번호, 보낸사람 번호가 일치하는 2명에게 웹소켓(실시간)으로 메세지 전달
	...
	// --> 채팅장 화면(SockJS 객체가 있는 화면)을 벗어나면 연결 종료
	@Override
...
/* WebSocket
- 브라우저와 웹서버간의 전이중통신을 지원하는 프로토콜(양방향통신)
	각 브라우저들간 통신 사이에 존재하는 웹 서버
- HTML5버전부터 지원하는 기능이다.
