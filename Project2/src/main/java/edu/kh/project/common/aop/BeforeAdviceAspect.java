...
@Aspect // 공통 관심사(advice) + 포인터 컷 작성된 클래스임을 명시(AOP코드)
public class BeforeAdviceAspect {
	...
	// CommonPointcut 클래스의 메서드중
	// serviceImplPointcut() 메서드에 작성된 pointcut 얻어옴
	@Before("CommonPointcut.serviceImplPointcut()")
	public void beforeServiceLog(JoinPoint jp) {
								// JoinPoint 객체 : 부가 기능이 적용되는 대상 객체의 클래스명, 메서드명, 파라미터 등을 얻을 수 있는 메서드
		
		String str = "---------------------------------------------------------------------------------";
		
		// jp.getTarget() : Pointcut으로 지정된 대상 객체
		// getClass().getSimpleName() : 클래스의 간단한 이름을 얻어옴
