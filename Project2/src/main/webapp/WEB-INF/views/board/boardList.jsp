        ...
        <section class="board-list">
            ...
            <div class="list-wrapper">
                <table class="list-table">
                    ...
                    <tbody>
                        <c:choose>
                            <c:when test="${empty boardList}"> <%-- 게시글 목록 조회 결과가 비어있다면 --%>
                                <tr>
                                    <th colspan="6">게시글이 존재하지 않습니다.</th>
                                </tr>
                            </c:when>

                            <c:otherwise> <%-- 게시글 목록 조회 결과가 있다면--%>
                                
                                <c:forEach var="board" items="${boardList}">
                                    <tr>
                                        <td>${board.boardNo}</td>
                                        <td>
                                            <c:if test="${not empty board.thumbnail}"> <%-- 썸네일이 있을 경우만 출력 --%>
                                                <img class="list-thumbnail" src="${board.thumbnail}">
                                            </c:if>

                                            <%-- a href="/board/1/1500?cp=1"  
                                                /board/{boardCode}/{boardNo}/?cp={pagination.curruntPage}
                                            --%>
