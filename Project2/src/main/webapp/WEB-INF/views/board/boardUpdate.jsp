...
<body>
    <main>
        ...
        <form action="update" method="POST" enctype="multipart/form-data" class="board-write" id="boardWriteForm" onsubmit="return updateValidate()">  
            ...
            <!-- imageList에 존재하는 이미지 순서에 따라 변수를 선언 -->
            <c:forEach items="${board.imageList}" var="img">
                <c:choose>
                    <c:when test="${img.imageOrder == 0}">
                        <c:set var="thumbnail" value="${img.imagePath}${img.imageReName}" />   <%-- page scope --%>          
                    </c:when>

                    <c:when test="${img.imageOrder == 1}">
                        <c:set var="img1" value="${img.imagePath}${img.imageReName}" />   <%-- page scope --%>          
                    </c:when>

                    <c:when test="${img.imageOrder == 2}">
                        <c:set var="img2" value="${img.imagePath}${img.imageReName}" />   <%-- page scope --%>          
                    </c:when>

                    <c:when test="${img.imageOrder == 3}">
                        <c:set var="img3" value="${img.imagePath}${img.imageReName}" />   <%-- page scope --%>          
                    </c:when>

                    <c:when test="${img.imageOrder == 4}">
                        <c:set var="img4" value="${img.imagePath}${img.imageReName}" />   <%-- page scope --%>          
                    </c:when>
