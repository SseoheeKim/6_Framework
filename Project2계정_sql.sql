...
-- 특정 게시글 이미지 모두 조회
SELECT * FROM BOARD_IMG WHERE BOARD_NO = 1998
ORDER BY IMG_ORDER;
...
-- 댓글 삽입
INSERT INTO "COMMENT" VALUES(SEQ_COMMENT_NO.NEXTVAL, '부모 댓글1', DEFAULT, DEFAULT, 15, 1998, null);
INSERT INTO "COMMENT" VALUES(SEQ_COMMENT_NO.NEXTVAL, '부모 댓글2', DEFAULT, DEFAULT, 10, 1998, null);

INSERT INTO "COMMENT" VALUES(SEQ_COMMENT_NO.NEXTVAL, '대댓글1', DEFAULT, DEFAULT, 10, 1998, 2013);
INSERT INTO "COMMENT" VALUES(SEQ_COMMENT_NO.NEXTVAL, '대댓글2', DEFAULT, DEFAULT, 10, 1998, 2013);
INSERT INTO "COMMENT" VALUES(SEQ_COMMENT_NO.NEXTVAL, '대댓글3', DEFAULT, DEFAULT, 15, 1998, 2013);

INSERT INTO "COMMENT" VALUES(SEQ_COMMENT_NO.NEXTVAL, '대댓글4', DEFAULT, DEFAULT, 10, 1998, 2014);
INSERT INTO "COMMENT" VALUES(SEQ_COMMENT_NO.NEXTVAL, '대댓글5', DEFAULT, DEFAULT, 10, 1998, 2014);
COMMIT;

-- 특정 게시글 댓글 모두 조회(계층형 쿼리)
...
1 : FROM (+JOIN)
4 : WHERE
2 : START WITH  -> 상위 타입(최상위 부모)으로 사용될 행을 지정 (서브쿼리로 지정 가능)
3 : CONNECT BY  -> 상위 타입과 하위 타입 사이의 관계를 규정   			
6 : ORDER SIBLINGS BY -> 계층 구조 정렬


- CONNECT BY :  PRIOR(이전의) 연산자와 같이 사용하여
...
SELECT LEVEL, C.* FROM (SELECT COMMENT_NO, COMMENT_CONTENT, 
						...
						FROM "COMMENT"
						JOIN MEMBER USING (MEMBER_NO)
						WHERE BOARD_NO = 1998 AND COMMENT_DEL_FL = 'N') C
...
ROLLBACK;
SELECT * FROM board WHERE BOARD_NO = 1998;
...
-- 한번에 여러 행 삽입 방법
-- INSERT ALL : 한번에 여러 행 삽입 가능(단, 시퀀스 사용 불가)
...
-- 제목 + 내용 검색
--AND (BOARD_TITLE LIKE '%10%' OR BOARD_CONTENT LIKE '%23%')
-- 작성자 닉네임
...
COMMENT ON COLUMN "MESSAGE"."READ_FL" IS '읽음 여부'; -- 'N' 안읽음

COMMENT ON COLUMN "MESSAGE"."SEND_TIME" IS '메세지 보낸 시간';

COMMENT ON COLUMN "MESSAGE"."SENDER_NO" IS '보낸 회원의 번호';

COMMENT ON COLUMN "MESSAGE"."CHATTING_NO" IS '채팅방 번호'; -- 채팅방으로 메세지 전송
